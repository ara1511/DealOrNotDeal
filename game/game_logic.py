...
class DealOrNoDeal:
    def __init__(self):
        ...
        self.main_case = None
        self.opened_cases = []
        self.cases_to_open = 5
        self.main_case_selected = False        
        self.remaining_cases = {i: value for i, value in enumerate(self.cases, start=1)}
        self.opened_cases = []
        self.bank_offers = []
        self.rounds = [6, 5, 4, 3, 2, 1, 1]  # Rondas de selección de maletines
...
def select_main_case(self, case_number):
        if not self.main_case_selected:
            raise ValueError("El maletín principal ya ha sido seleccionado.")
            self.main_case = case_number
            self.main_case_selected = True
            
def select_case(self, case_number):
    if not self.main_case_selected:
        raise ValueError("Debes seleccionar el maletín principal primero.")
    if case_number in self.opened_cases:
        raise ValueError("Este maletín ya ha sido abierto.")
    self.opened_cases.append(case_number)
    self.cases_to_open -= 1


def open_case(self, case_number):
        if not self.main_case_selected:
            raise ValueError("Primero debes seleccionar tu maletín principal.")
        if case_number in self.remaining_cases:
            case_value = self.remaining_cases.pop(case_number)
            self.opened_cases.append((case_number, case_value))
            self.cases_to_open -= 1
...
def next_round(self):
        if self.cases_to_open > 0:
            raise ValueError("Aún debes abrir más maletines antes de continuar a la siguiente ronda.")
        self.cases_to_open = 5 
